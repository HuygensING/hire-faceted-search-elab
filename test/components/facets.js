	...
	it("should render properly", () => {
		...
		function setup() {
			...
			let props = {
				...
				facetList: ["foo", "bar"],
				results: {last: {facets: [{title: "foo", type: "LIST"}, {title: "bar", type: "LIST"}, {title: "do-not-show"}]}},
		...
		expect(childFacets.length).toBe(2);
		let [fooFacet, barFacet] = childFacets;
		expect(fooFacet.type).toBe(ListFacet);
		expect(barFacet.type).toBe(ListFacet);
		expect(fooFacet.props.data).toEqual({title: "foo", type: "LIST"});
		expect(barFacet.props.data).toEqual({title: "bar", type: "LIST"});
